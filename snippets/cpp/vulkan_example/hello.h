...
private:
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdBindDescriptorSets(m_commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 0, 1, m_descriptorSets.data(), 0, nullptr);
    ...
    void updateUniformBuffer() {
    	...
    	float time = std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - m_since_start).count() / 1000.0f;
    ...
    void createDescriptorPool()
    {
        ...
        if (vkCreateDescriptorPool(m_device, &poolInfo, nullptr, &m_descriptorPool) != VK_SUCCESS) {
            throw string("failed to create descriptor pool!");
        }
    }
    ...
    void createDescriptorSets()
    {
        ...
        if (vkAllocateDescriptorSets(m_device, &allocInfo, m_descriptorSets.data()) != VK_SUCCESS) {
            throw string("failed to allocate descriptor sets!");
        }
        ...
        descriptorWrite[0].dstSet = m_descriptorSets[0];
        ...
        descriptorWrite[1].dstSet = m_descriptorSets[0];
    ...
    void createTextureImage() {
        ...
        stbi_uc* pixels = stbi_load("../extern/texture.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
        VkDeviceSize imageSize = texWidth * texHeight * 4;

        if (!pixels) {
            throw string("failed to load texture image!");
        }
        ...
        createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, 
            VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_textureImage, m_textureImageMemory);
    ...
    void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
        ...
        if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
            ...
            throw string("unsupported layout transition!");
        }
    ...
    void createTextureSampler() {
        ...
        if (vkCreateSampler(m_device, &samplerInfo, nullptr, &m_textureSampler) != VK_SUCCESS) {
                throw string("failed to create texture sampler!");
        }
    }
